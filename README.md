...
Please ensure that you are playing on the USA or EUR version of Majora's Mask 3D, as the randomizer may not support Asian versions. You can use either the cartridge version or the installed digital version. In all instructions below, if a directory doesn't exist, just create it.
...
To generate a playthrough, run Majora's Mask 3D Randomizer from the homebrew launcher. Navigate the menus to choose the settings for your playthrough, make sure that `Game Settings` match your copy of the game, and finish by selecting ```Generate Randomizer```. After the app has finished, launch Majora's Mask 3D from the home menu and enjoy your playthough!

If the 3DS crashes when you launch the game, you probably need to update Luma3DS, or might need to correct something in `Game Settings` before generating a new randomizer.

### On Citra
Use Citra to launch ```MM3D_Randomizer.3dsx```. Navigate the menus to choose the settings for your playthrough, make sure that `Game Settings` match your copy of the game, and finish by selecting ```Generate Randomizer```. After the app has finished, on the Citra window, click on ```File->Open Citra Folder```. From this folder, navigate to ```\sdmc\luma\titles\0004000000125500``` (or ```\sdmc\luma\titles\0004000000125600``` if playing on a European copy,) and you should find files called ```code.ips```, ```exheader.bin```, and a folder called ```romfs```. In the Citra window, right-click on the installed copy of Majora's Mask 3D, and choose ```Open Mods Location```. (If you haven't installed MM3D, you can go to ```File->Open Citra Folder``` and then open up ```\load\mods\0004000000125500``` or ```\load\mods\0004000000125600```.) Copy ```code.ips```, ```exheader.bin```, and ```romfs``` into this folder. Now, launch Majora's Mask 3D and enjoy your playthrough!
